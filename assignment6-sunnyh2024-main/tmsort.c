...
/** global variable that keeps track of thread counts */
...
/** struct to be passed into thread function as args */
...
/** function to be passed into pthread_create */
void *threadFunction(void *args) {
	thread_args_t *thread_arg = (thread_args_t *)args;
	merge_sort_aux(thread_arg->target, thread_arg->from, thread_arg->to, thread_arg->nums);
  return 0;
}
...
void merge_sort_aux(long nums[], int from, int to, long target[]) {
  ...
  if(NUM_THREADS > 0) {
    ...
    thread_arg.nums = nums;
    thread_arg.from = from;
    thread_arg.to = mid;
    thread_arg.target = target;
    
    pthread_create(&tid, NULL, threadFunction, &thread_arg);
